...
Run_IgScan_FullWorkflow <- function(sample_paths, sample_labels, Evalue_cutoff = NULL, annotate_C = TRUE, threads = 1, threads_IgBlast = 1, case_labels = NULL, input_format, analysis_mode = "single", material_type = "rna", v_primer = "full_length", data_type = "single_cell", min_reads = 2, remove_tmp = TRUE, outputDir = NULL, hc_similarity_cutoff = 0.2, hc_mode = "average", cdr3_mode = "nt", cdr3_InDel_correction_mode = "soft_filter", annotate_CLL_immGen = FALSE){
  ...
  annotated_df_list <- Run_IgScan_Annotation(sample_labels = sample_labels, case_labels = case_labels, input_format = input_format, outputDir = outputDir, analysis_mode = analysis_mode, material_type = material_type, v_primer = v_primer, data_type = data_type, min_reads = min_reads, remove_tmp = remove_tmp, hc_similarity_cutoff = hc_similarity_cutoff, hc_mode = hc_mode, cdr3_mode = cdr3_mode, cdr3_InDel_correction_mode = cdr3_InDel_correction_mode, annotate_CLL_immGen = annotate_CLL_immGen, summary_file = summary_file, threads = threads)
