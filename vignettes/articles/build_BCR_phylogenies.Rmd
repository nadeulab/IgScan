...
s <- system.file("extdata/igscan_test_10xSeurat_IgScan_sample4.rds", package = "IgScan", mustWork = T)
so <- readRDS(s)
```
...
```{r dowser_phylo, message=FALSE, warning=FALSE, tidy=FALSE, fig.height=5, fig.width=10}
...
As you can see in the plot above, we have produced BCR-based phylogenetic trees for all the clones with at least two different nucleotide sequences (`minseq = 2`). This has left us with two different trees, for different clonotypes (C1 and C2 in the Seurat object).
